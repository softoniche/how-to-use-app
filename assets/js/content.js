...
// is a string id from i18n.js, so a screen is written once and reads in all
// eleven languages.
//
// To change the guide, edit this file (and tools/sync_translations.py for new
// wording) — nothing else needs to be touched.
//
//   tone   primary | success | info      the accent the screen is painted in
//   full   true when a screen, block, page or item belongs to the full guide
//          only. With ?full=0 — what the app asks for when its extra features
//          are switched off — everything marked `full` disappears.
//   lite   the other way round: shown only with ?full=0.
...
//   slides    one screenshot at a time with a progress bar, ending on "Got it"
...
//   note      a tip; `variant: 'safe'` paints it as reassurance
...
window.GUIDE_CONTENT = (function () {
  ...
  var journey = [
    { title: 'connect_title', desc: 'journey_connect_desc' },
    { title: 'ready_open_title', desc: 'ready_open_desc' },
    { title: 'saved_open_title', desc: 'saved_open_desc' },
  ];

  // What someone about to link their account worries about, answered before
  // they have to ask.
  var faq = {
    type: 'faq',
    label: 'faq_title',
    items: [
      { q: 'faq_safe_q', a: 'faq_safe_a' },
      { q: 'faq_phone_q', a: 'faq_phone_a' },
      { q: 'faq_notice_q', a: 'faq_notice_a' },
      { q: 'faq_loading_q', a: 'faq_loading_a' },
    ],
  };

  var connectOption = {
    type: 'options',
    items: [{ icon: 'link', title: 'home_connect_title', desc: 'home_connect_desc', go: 'connect', accent: true }],
  };

  var badges = [
    { icon: 'shield-check', text: 'trust_official_title' },
    { icon: 'lock', text: 'trust_local_title', full: true },
    { icon: 'phone', text: 'privacy_device_title', lite: true },
  ];

  var officialNote = { type: 'note', variant: 'safe', icon: 'shield-check', title: 'trust_official_title', desc: 'trust_official_desc' };

  return {
    // Where the guide opens: `home` from Settings, `onboarding` on a fresh
    // install. ?screen=<id> can open it somewhere else.
    home: 'home',
    onboarding: 'welcome',
    ...
    screens: {
      ...
      // ── From Settings: everything on one page to scroll. ──
      home: {
        tone: 'primary',
        label: 'guide_title',
        blocks: [
          { type: 'hero', title: 'hero_title', subtitle: 'hero_subtitle', badges: badges, full: true },
          { type: 'hero', title: 'hero_title_lite', subtitle: 'hero_subtitle_lite', badges: badges, lite: true },
          {
            type: 'showcase',
            full: true,
            items: [
          ...
          connectOption,
          {
            ...
            title: 'trust_title',
            items: [
              { icon: 'link', title: 'trust_official_title', desc: 'trust_official_desc' },
              { icon: 'lock', title: 'trust_local_title', desc: 'trust_local_desc', full: true },
              { icon: 'phone', title: 'privacy_device_title', desc: 'privacy_device_desc', lite: true },
              { icon: 'shield', title: 'privacy_control_title', desc: 'trust_control_desc' },
            ],
          },
        ],
      },
      ...
      connect: {
        tone: 'primary',
        icon: 'link',
        // The same words the user tapped, so the answer plainly belongs to it.
        ...
        blocks: [
          {
            type: 'options',
            items: [
              { icon: 'phone', title: 'connect_phone_title', desc: 'connect_here_where', go: 'connect_here', badge: 'most_common' },
      ...
      connect_here: {
        ...
        blocks: [
          ...
          // there is none, nobody sees a button that does nothing. Under the
          // steps, so the way on is the first thing on the screen.
      ...
      // ── Where deleted, edited and view-once messages end up. ──
      saved: {
        full: true,
        tone: 'primary',
        icon: 'bookmark',
        title: 'toolbar_saved_title',
        subtitle: 'saved_open_desc',
        blocks: [
          {
            type: 'steps',
            items: [
              { title: 'ready_open_title', desc: 'ready_open_desc' },
              { title: 'saved_open_title', desc: 'saved_where_desc', image: 'assets/img/icon-saved.jpg' },
              { title: 'saved_view_title', desc: 'saved_view_desc' },
            ],
          },
          { type: 'done' },
        ],
